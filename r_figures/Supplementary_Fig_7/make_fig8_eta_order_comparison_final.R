...
source_dir <- normalizePath(
  Sys.getenv("FIG8_SOURCE_DIR", unset = file.path(script_dir, "data")),
  winslash = "/",
  mustWork = FALSE
)
out_dir <- normalizePath(
  Sys.getenv("FIG8_OUTPUT_DIR", unset = script_dir),
  winslash = "/",
  mustWork = FALSE
)
...
p_a <- ggplot(stage, aes(x = phase5, y = eta_mean, colour = Model)) +
  ...
  labs(
    x = NULL,
    y = eta_italic_sym,
    title = "<b>Stage-resolved <i>η</i> across order representations</b>",
    tag = "a"
  ) +
  guides(colour = "none") +
  theme_nature() +
...
p_b <- ggplot(variance_plot_data, aes(x = Phase, y = Variance, colour = Model, group = Model)) +
  ...
  labs(
    ...
    title = "<b>Cross-seizure variability</b>",
    tag = "b"
  ) +
...
auc_label_df <- roc_summary %>%
  arrange(Model) %>%
  mutate(
    label = paste0(auc_short_labels[as.character(Model)], " ", formatC(auc, format = "f", digits = 3)),
    x = 0.95,
    y = c(0.25, 0.16, 0.07)
  )
