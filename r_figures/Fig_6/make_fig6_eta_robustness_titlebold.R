...
theme_heat <- function(base_size = 6.8, base_family = "Arial") {
  theme_minimal(base_size = base_size, base_family = base_family) +
    theme(
      axis.title = element_text(size = base_size + 0.2),
      axis.text = element_text(size = base_size - 0.1, colour = "#252525"),
      plot.title = element_text(size = base_size + 0.9, face = "bold", hjust = 0, colour = "#202124"),
...
p_a <- ggplot(df, aes(x = threshold_f, y = window_f, fill = neg_log10_p)) +
  ...
  geom_text(aes(label = p_num, colour = text_colour_for(neg_log10_p, 3.78)), size = 2.15, family = "Arial") +
  ...
  labs(
    ...
    y = "Window length",
    title = paste0("Discriminative power of ", intToUtf8(0x03b7))
  ) +
  theme_heat()

p_b <- ggplot(df, aes(x = threshold_f, y = window_f, fill = mean_ictal_minus_pre)) +
  geom_tile(colour = "white", linewidth = 0.55) +
  geom_text(aes(label = diff_num, colour = text_colour_for(mean_ictal_minus_pre, 0.014)), size = 2.0, family = "Arial") +
  scale_colour_identity() +
  scale_fill_gradientn(
    colours = pal_sig,
  ...
  labs(
    x = "PLV threshold percentile",
    y = NULL,
    title = paste0("Effect size: mean ictal ", intToUtf8(0x03b7), " - mean pre-ictal ", intToUtf8(0x03b7))
  ) +
  theme_heat()
...
out_base <- file.path(out_dir, "Fig6_eta_robustness_titlebold")
