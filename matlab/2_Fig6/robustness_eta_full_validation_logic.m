...
%% 1. 参数设置与路径
outDir = 'E:\wcldematlab\keep\new_idea\8 - n_u_v\2_Fig6\3\Robustness_Results_eta';
if ~exist(outDir, 'dir'), mkdir(outDir); end

winLenRange    = [1, 2, 3, 4, 5];      % 窗口长度（秒）
thresholdRange = 0.40 : 0.05 : 0.70;   % PLV 阈值分位数
nWin   = numel(winLenRange);
nThres = numel(thresholdRange);
...
for c = 1:nCase
    ...
    for i = 1:nWin
        ...
        for j = 1:nThres
            currThres = thresholdRange(j);
            eta_vals = nan(numel(target_indices), 1);

            for k = 1:numel(target_indices)
                HE = plv_to_hyperedges_quantile(plv_cache{k}, currThres);
...
h1.XLabel = 'PLV Threshold Percentile';
...
h2.XLabel = 'PLV Threshold Percentile';
...
save(fullfile(outDir, 'Robustness_eta_Data_Workspace.mat'), ...
    ...
    'winLenRange', 'thresholdRange', 'caseInfo');
...
function HE = plv_to_hyperedges_quantile(plvMat, thresPct)
    dc = size(plvMat, 1);
