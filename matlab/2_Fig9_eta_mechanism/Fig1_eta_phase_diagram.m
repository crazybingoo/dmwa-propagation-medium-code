...
imagesc(c_range, h_range, R.eta_mean);
axis xy;
colormap(parula);
cb = colorbar;
cb.Label.String = '\eta';
...
title('\eta phase diagram', 'FontName', 'Arial', 'FontSize', 13, 'FontWeight', 'bold');

set(gca, 'FontName', 'Arial', 'FontSize', 11, 'LineWidth', 1.1, ...
    'Box', 'off', 'TickDir', 'out');

text(0.12, 0.12, 'Diffuse / hetero', 'Color', [0.2 0.2 0.2], 'FontSize', 10, 'FontWeight', 'bold');
text(0.70, 0.85, 'Concentrated / homogeneous', 'Color', [1 1 1], 'FontSize', 10, 'FontWeight', 'bold');

exportgraphics(fig, fullfile(outDir, 'Fig1_eta_phase_diagram.png'), 'Resolution', 600);
exportgraphics(fig, fullfile(outDir, 'Fig1_eta_phase_diagram.pdf'), 'ContentType', 'vector');
savefig(fig, fullfile(outDir, 'Fig1_eta_phase_diagram.fig'));
