...
# Original is the accent orange-red so the main method is visually foregrounded.
model_palette <- c(
  "Original" = "#D6603D",
...
base_file <- file.path(out_dir, "Fig7_eta_ablation_components_method_highlight")
...
write.csv(post_checks, file.path(out_dir, "Fig7_postplot_export_checks_method_highlight.csv"), row.names = FALSE)

writeLines(
  c(
    "Fig. 7 method-highlighting note",
    "",
    "Old versions preserved: Fig7_eta_ablation_components.*, Fig7_eta_ablation_components_aligned_palette.* and Fig7_eta_ablation_components_nc_orangered.*",
    "Method-highlighting version: Fig7_eta_ablation_components_method_highlight.*",
    "Palette: Original #D6603D, Coverage only #4778A8, DegreeBias only #7A828C.",
    "Rationale: the main method is foregrounded with the orange-red accent, while the ablated variants are visually pushed back with blue and gray. The panel b legend remains above the plot area to avoid overlap with the upper line.",
    "Data and statistics are unchanged from the audited Fig. 7 source tables."
  ),
  file.path(out_dir, "Fig7_palette_alignment_note_method_highlight.txt"),
  useBytes = TRUE
)

if (any(post_checks$status == "FAIL")) {
  stop("Export checks failed. Inspect Fig7_postplot_export_checks_method_highlight.csv")
}

message("Method-highlight Fig. 7 complete: ", base_file)
