%% Fig3 切片图：lambda_1
% 五条线固定为 0.1:0.2:0.9
% 左图：固定 h，观察 lambda_1 随 c 变化
% 右图：固定 c，观察 lambda_1 随 h 变化
...
for i = 1:nSlice
    plot(ax1, c_range, lambda1_mean(h_idx(i), :), '-o', ...
...
ylabel(ax1, '\lambda_1', 'FontName', fontName, 'FontSize', fontAxis, 'FontWeight', 'bold');
title(ax1, 'Fixed h: \lambda_1 vs c', 'FontName', fontName, 'FontSize', 10.5, 'FontWeight', 'bold');
...
for i = 1:nSlice
    plot(ax2, h_range, lambda1_mean(:, c_idx(i)), '-o', ...
        'LineWidth', 1.8, 'MarkerSize', 5.0, ...
        'Color', lineColors(i, :), ...
        'MarkerFaceColor', 'w', ...
...
ylabel(ax2, '\lambda_1', 'FontName', fontName, 'FontSize', fontAxis, 'FontWeight', 'bold');
title(ax2, 'Fixed c: \lambda_1 vs h', 'FontName', fontName, 'FontSize', 10.5, 'FontWeight', 'bold');
...
pngFile = fullfile(outDir, 'Fig3_lambda1_representative_slices_5lines.png');
pdfFile = fullfile(outDir, 'Fig3_lambda1_representative_slices_5lines.pdf');
figFile = fullfile(outDir, 'Fig3_lambda1_representative_slices_5lines.fig');
