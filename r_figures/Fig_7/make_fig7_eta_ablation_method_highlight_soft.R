...
# Original is the accent but in a softer dusty coral so the main method is
# foregrounded without feeling too saturated.
model_palette <- c(
  "Original" = "#E16A54",
...
base_file <- file.path(out_dir, "Fig7_eta_ablation_components_method_highlight_soft")
...
write.csv(post_checks, file.path(out_dir, "Fig7_postplot_export_checks_method_highlight_soft.csv"), row.names = FALSE)

writeLines(
  c(
    "Fig. 7 method-highlighting note",
    "",
    "Old versions preserved: Fig7_eta_ablation_components.*, Fig7_eta_ablation_components_aligned_palette.*, Fig7_eta_ablation_components_nc_orangered.* and Fig7_eta_ablation_components_method_highlight.*",
    "Soft method-highlighting version: Fig7_eta_ablation_components_method_highlight_soft.*",
    "Palette: Original #E16A54, Coverage only #4778A8, DegreeBias only #7A828C.",
    "Rationale: the main method is foregrounded with a softer dusty coral accent, while the ablated variants are visually pushed back with blue and gray. The panel b legend remains above the plot area to avoid overlap with the upper line.",
    "Data and statistics are unchanged from the audited Fig. 7 source tables."
  ),
  file.path(out_dir, "Fig7_palette_alignment_note_method_highlight_soft.txt"),
  useBytes = TRUE
)

if (any(post_checks$status == "FAIL")) {
  stop("Export checks failed. Inspect Fig7_postplot_export_checks_method_highlight_soft.csv")
}

message("Soft method-highlight Fig. 7 complete: ", base_file)
