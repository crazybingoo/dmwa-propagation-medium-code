...
%% 2) 图 4：代表性切片
% 左图：固定 h 看 eta 随 c 变化
% 右图：固定 c 看 eta 随 h 变化

h_slices = [0.10, 0.30, 0.50, 0.70, 0.90];
c_slices = [0.10, 0.30, 0.50, 0.70, 0.90];

h_idx = zeros(size(h_slices));
c_idx = zeros(size(c_slices));
for i = 1:numel(h_slices)
    [~, h_idx(i)] = min(abs(h_range - h_slices(i)));
end
for i = 1:numel(c_slices)
    [~, c_idx(i)] = min(abs(c_range - c_slices(i)));
end

fig = figure('Color', 'w', 'Position', [120, 120, 900, 400]);
t = tiledlayout(1,2, 'TileSpacing', 'compact', 'Padding', 'compact');

ax1 = nexttile(t, 1); hold(ax1, 'on');
for i = 1:numel(h_idx)
    plot(ax1, c_range, R.eta_mean(h_idx(i), :), '-o', 'LineWidth', 2.0, 'MarkerSize', 5, ...
        'DisplayName', sprintf('h = %.2f', h_range(h_idx(i))));
end
xlabel(ax1, 'Core concentration c', 'FontName', 'Arial', 'FontSize', 12, 'FontWeight', 'bold');
ylabel(ax1, '\eta', 'FontName', 'Arial', 'FontSize', 12, 'FontWeight', 'bold');
title(ax1, 'Fixed h: \eta vs c', 'FontName', 'Arial', 'FontSize', 12, 'FontWeight', 'bold');
set(ax1, 'FontName', 'Arial', 'FontSize', 11, 'LineWidth', 1.1, 'Box', 'off', 'TickDir', 'out');
legend(ax1, 'Location', 'best', 'Box', 'off');

ax2 = nexttile(t, 2); hold(ax2, 'on');
for i = 1:numel(c_idx)
    plot(ax2, h_range, R.eta_mean(:, c_idx(i)), '-o', 'LineWidth', 2.0, 'MarkerSize', 5, ...
        'DisplayName', sprintf('c = %.2f', c_range(c_idx(i))));
end
xlabel(ax2, 'Homogenization h', 'FontName', 'Arial', 'FontSize', 12, 'FontWeight', 'bold');
ylabel(ax2, '\eta', 'FontName', 'Arial', 'FontSize', 12, 'FontWeight', 'bold');
title(ax2, 'Fixed c: \eta vs h', 'FontName', 'Arial', 'FontSize', 12, 'FontWeight', 'bold');
set(ax2, 'FontName', 'Arial', 'FontSize', 11, 'LineWidth', 1.1, 'Box', 'off', 'TickDir', 'out');
legend(ax2, 'Location', 'best', 'Box', 'off');

exportgraphics(fig, fullfile(outDir, 'Fig4_eta_representative_slices.png'), 'Resolution', 600);
exportgraphics(fig, fullfile(outDir, 'Fig4_eta_representative_slices.pdf'), 'ContentType', 'vector');
savefig(fig, fullfile(outDir, 'Fig4_eta_representative_slices.fig'));
