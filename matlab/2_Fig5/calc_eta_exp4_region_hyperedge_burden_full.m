%% calc_eta_exp4_region_hyperedge_burden_full.m
% =========================================================
% 实验 4：区域与超边贡献分析
%
% 问题：eta 的变化由谁承担？
...
%   3) ALL_CASES_exp4_window_region_contribution.csv
%   4) ALL_CASES_exp4_stage_region_contribution.csv
...
%   9) GROUP_exp4_region_phase_stats.csv
%  10) GROUP_exp4_macro_role_phase_stats.csv
%  11) FIG_exp4_eta_burden_summary.png / .pdf / .fig
%
% 说明：
...
for f = 1:numel(matFiles)
    ...
    for t = 1:T
        ...
        % 4.2.2 窗级：SOZ/PZ/NIZ 区域承担
        regionProps = [[HEinfo.prop_SOZ]', [HEinfo.prop_PZ]', [HEinfo.prop_NIZ]'];
        for r = 1:3
            wr = regionProps(:, r);
            row = make_window_region_row(case_id, t, phase5{t}, regionNames{r}, eta0, eta_saved, wr, M, EPS0);
...
writetable(ALL_windowRegionTable, fullfile(outDir, 'ALL_CASES_exp4_window_region_contribution.csv'));
...
writetable(ALL_stageRegionTable, fullfile(outDir, 'ALL_CASES_exp4_stage_region_contribution.csv'));
...
writetable(GROUP_regionStats, fullfile(outDir, 'GROUP_exp4_region_phase_stats.csv'));
writetable(GROUP_macroRoleStats, fullfile(outDir, 'GROUP_exp4_macro_role_phase_stats.csv'));

%% 8) 主要总结图：一张图，三个机制面板
plot_exp4_summary_figure(ALL_stageRegionTable, ALL_stageMacroRoleTable, ...
    phaseOrder, phaseShort, regionNames, macroOrder, roleOrder, outDir);

%% 9) 保存 MAT
save(fullfile(outDir, 'ALL_CASES_exp4_region_hyperedge_contribution_results.mat'), ...
    'ALL_hyperedgeTable', ...
...
function row = make_window_region_row(case_id, window_idx, phase5, region, eta0, eta_saved, weights, M, EPS0)
    weights = weights(:);
    nEff = sum(weights);
    ...
    row = {case_id, window_idx, phase5, region, eta0, eta_saved, ...
           nEff, eta_resource_contrib, eta_lambda_contrib, eta_proxy_delta, eta_abs_burden, ...
           directional_load, directional_load_share, source_load, source_load_share, ...
           sink_load, sink_load_share, mean_flow_bias};
end
...
function names = window_region_varnames()
    names = {'case_id','window_idx','phase5','region','eta_recomputed','eta_saved', ...
        'n_hyperedges_effective','eta_resource_contrib','eta_lambda_contrib', ...
        'eta_proxy_delta','eta_abs_burden','directional_load','directional_load_share', ...
        'source_load','source_load_share','sink_load','sink_load_share','mean_flow_bias'};
end
...
function Tout = summarize_stage_region(Twin, phaseOrder, regionNames)
    ...
    metrics = {'eta_recomputed','eta_saved','n_hyperedges_effective', ...
    ...
    for i = 1:numel(caseIDs)
        for r = 1:numel(regionNames)
            for p = 1:numel(phaseOrder)
                ...
                row = {caseIDs{i}, regionNames{r}, phaseOrder{p}, sum(idx)};
    ...
    names = {'case_id','region','phase5','n_windows'};
    for m = 1:numel(metrics)
...
function G = summarize_group_region(Tstage, phaseOrder, regionNames)
    metricNames = {'eta_resource_contrib_mean','eta_lambda_contrib_mean','eta_proxy_delta_mean', ...
        'eta_abs_burden_mean','directional_load_share_mean','source_load_share_mean', ...
        'sink_load_share_mean','mean_flow_bias_mean'};
    rows = {};
...
function plot_exp4_summary_figure(Tregion, TmacroRole, phaseOrder, phaseShort, regionNames, macroOrder, roleOrder, outDir)
    % 一张图：A 区域承担；B 宏观状态组合承担；C source/sink/balanced 承担
    ...
    %% A: SOZ/PZ/NIZ eta_abs_burden
    nexttile; hold on;
    for r = 1:numel(regionNames)
        [y,e] = group_mean_sem(Tregion, 'region', regionNames{r}, 'eta_abs_burden_mean', phaseOrder);
    ...
    title('A  Regional eta burden', 'FontWeight','bold');
    ...
    format_phase_axis(phaseShort);
    ylabel('eta burden, abs proxy', 'FontWeight','bold');
    title('B  Macro-state combination', 'FontWeight','bold');
    ...
    format_phase_axis(phaseShort);
    ylabel('eta burden, abs proxy', 'FontWeight','bold');
    title('C  Source/sink role', 'FontWeight','bold');
    legend(roleOrder, 'Box','off', 'Location','best');

    title(tl, 'Experiment 4: regional and hyperedge-level contributors of eta', ...
        'FontName','Arial', 'FontSize',13, 'FontWeight','bold');

    exportgraphics(fig, fullfile(outDir, 'FIG_exp4_eta_burden_summary.png'), 'Resolution', 600);
    exportgraphics(fig, fullfile(outDir, 'FIG_exp4_eta_burden_summary.pdf'), 'ContentType','vector');
    savefig(fig, fullfile(outDir, 'FIG_exp4_eta_burden_summary.fig'));
end
