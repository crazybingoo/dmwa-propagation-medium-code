...
# Original remains the accent, but all three series are muted a little more so
# the page reads calmer and closer to a Nature-style restrained palette.
model_palette <- c(
  "Original" = "#D96A63",
  "Coverage only" = "#5A84AF",
  "DegreeBias only" = "#8B919A"
)
...
theme_nature <- function(base_size = 6.6, base_family = "Arial") {
  theme_classic(base_size = base_size, base_family = base_family) +
    theme(
      ...
      axis.text = element_text(size = base_size - 0.2, colour = "#3A3A3A"),
...
sig_short <- paired_stats %>%
  ...
  mutate(
    label = ifelse(p_value < 0.001, "Pre-Early\nP<0.001",
...
base_file <- file.path(out_dir, "Fig7_eta_ablation_components_method_highlight_muted")
...
write.csv(post_checks, file.path(out_dir, "Fig7_postplot_export_checks_method_highlight_muted.csv"), row.names = FALSE)

writeLines(
  c(
    "Fig. 7 method-highlighting note",
    "",
    "Old versions preserved: Fig7_eta_ablation_components.*, Fig7_eta_ablation_components_aligned_palette.*, Fig7_eta_ablation_components_nc_orangered.*, Fig7_eta_ablation_components_method_highlight.*, Fig7_eta_ablation_components_method_highlight_soft.* and Fig7_eta_ablation_components_method_highlight_softer.*",
    "Muted method-highlighting version: Fig7_eta_ablation_components_method_highlight_muted.*",
    "Palette: Original #D96A63, Coverage only #5A84AF, DegreeBias only #8B919A.",
    "Rationale: the main method is still foregrounded in coral-red, but all three series are slightly more muted so the page reads calmer; the panel b legend remains above the plot area to avoid overlap with the upper line.",
    "Data and statistics are unchanged from the audited Fig. 7 source tables."
  ),
  file.path(out_dir, "Fig7_palette_alignment_note_method_highlight_muted.txt"),
  useBytes = TRUE
)

if (any(post_checks$status == "FAIL")) {
  stop("Export checks failed. Inspect Fig7_postplot_export_checks_method_highlight_muted.csv")
}

message("Muted method-highlight Fig. 7 complete: ", base_file)
