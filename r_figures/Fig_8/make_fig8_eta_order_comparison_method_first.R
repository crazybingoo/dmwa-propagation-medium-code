...
base_file <- file.path(out_dir, "Fig8_eta_order_representation_comparison_method_first")
...
write.csv(post_checks, file.path(out_dir, "Fig8_postplot_export_checks_method_first.csv"), row.names = FALSE)
...
write.csv(figure_numeric_summary, file.path(out_dir, "Fig8_figure_numeric_summary_method_first.csv"), row.names = FALSE)
...
writeLines(
  ...
  file.path(out_dir, "Fig8_figure_contract_and_audit_note_method_first.txt"),
  useBytes = TRUE
)

if (any(post_checks$status == "FAIL")) {
  stop("Export checks failed; inspect Fig8_postplot_export_checks_method_first.csv")
}
