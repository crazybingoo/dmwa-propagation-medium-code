...
%% =========================================================
% 单独作图脚本：基于 calc_C.m 已经输出的结果文件进行作图
%
% 依赖输入：
%   ALL_CASES_stage5_C.csv
%   （该文件由 calc_C.m 运行后生成）
%
% 输出文件：
%   GROUP_stage5_C_boxplot_with_p_d.png
%   GROUP_stage5_C_boxplot_with_p_d.fig
%   GROUP_stage5_C_pairwise_with_d.csv
%% =========================================================
...
if ~exist(inFile, 'file')
    error('未找到结果文件: %s\n请先运行 calc_C.m，或检查 resultDir 是否正确。', inFile);
end

%% 1) 读取 stage5 结果
T = readtable(inFile);

reqVars = {'case_id','phase5','C_mean'};
...
writetable(pairwiseStats, fullfile(resultDir, 'GROUP_stage5_C_pairwise_with_d.csv'));
...
%% 8) 保存输出
pngFile = fullfile(resultDir, 'GROUP_stage5_C_boxplot_with_p_d.png');
figFile = fullfile(resultDir, 'GROUP_stage5_C_boxplot_with_p_d.fig');
...
fprintf('输出图片: %s\n', pngFile);
fprintf('输出FIG : %s\n', figFile);
fprintf('统计表  : %s\n', fullfile(resultDir, 'GROUP_stage5_C_pairwise_with_d.csv'));
fprintf('========================================\n');
