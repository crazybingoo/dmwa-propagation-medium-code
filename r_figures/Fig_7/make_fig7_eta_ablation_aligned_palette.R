...
# Aligned with the preceding manuscript palette:
# neutral black, Fig. 3/Fig. 4 blue, and the previously used NC-safe
# orange-red accent from the earlier manuscript figures.
model_palette <- c(
  "Original" = "#202124",
  "Coverage only" = "#4778A8",
  "DegreeBias only" = "#D6603D"
)
...
base_file <- file.path(out_dir, "Fig7_eta_ablation_components_nc_orangered")
...
write.csv(post_checks, file.path(out_dir, "Fig7_postplot_export_checks_nc_orangered.csv"), row.names = FALSE)

writeLines(
  c(
    "Fig. 7 palette alignment note",
    "",
    "Old versions preserved: Fig7_eta_ablation_components.* and Fig7_eta_ablation_components_aligned_palette.*",
    "NC orange-red version: Fig7_eta_ablation_components_nc_orangered.*",
    "Palette: Original #202124, Coverage only #4778A8, DegreeBias only #D6603D.",
    "Rationale: the new palette follows the manuscript's recurring neutral, muted blue and orange-red family used in preceding figures, instead of the earlier brown DegreeBias colour. The panel b legend is placed above the plotting area to avoid intersecting the upper line.",
    "Data and statistics are unchanged from the audited Fig. 7 source tables."
  ),
  file.path(out_dir, "Fig7_palette_alignment_note_nc_orangered.txt"),
  useBytes = TRUE
)

if (any(post_checks$status == "FAIL")) {
  stop("Export checks failed. Inspect Fig7_postplot_export_checks_nc_orangered.csv")
}

message("NC orange-red Fig. 7 complete: ", base_file)
