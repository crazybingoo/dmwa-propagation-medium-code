...
base_file <- file.path(out_dir, "Fig7_eta_ablation_components_method_highlight_softer")
...
write.csv(post_checks, file.path(out_dir, "Fig7_postplot_export_checks_method_highlight_softer.csv"), row.names = FALSE)
...
writeLines(
  c(
    ...
    "Softer method-highlighting version: Fig7_eta_ablation_components_method_highlight_softer.*",
    "Palette: Original #D96661, Coverage only #4778A8, DegreeBias only #7A828C.",
    "Rationale: the main method is foregrounded with a softer coral-red accent, while the ablated variants are visually pushed back with blue and gray. The panel b legend remains above the plot area to avoid overlap with the upper line.",
    "Data and statistics are unchanged from the audited Fig. 7 source tables."
  ),
  file.path(out_dir, "Fig7_palette_alignment_note_method_highlight_softer.txt"),
  useBytes = TRUE
)

if (any(post_checks$status == "FAIL")) {
  stop("Export checks failed. Inspect Fig7_postplot_export_checks_method_highlight_softer.csv")
}

message("Softer method-highlight Fig. 7 complete: ", base_file)
