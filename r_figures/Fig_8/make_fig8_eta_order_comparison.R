...
model_levels <- c("Low-Order(2-nodes)", "High-Order(3-nodes)", "DMW-HLG")
model_labels <- c(
...
paired_label_df <- paired_stats %>%
  ...
  left_join(
    stage_summary %>%
      group_by(Model) %>%
      summarise(y = max(q3_eta, mean_eta + ci95_eta, na.rm = TRUE) + 0.0065, .groups = "drop"),
    by = "Model"
  )
...
p_a <- ggplot(stage, aes(x = phase5, y = eta_mean, colour = Model)) +
  ...
  coord_cartesian(ylim = c(0.945, 1.008), clip = "off") +
  labs(
    x = NULL,
    y = expression(eta),
    title = expression("Stage-resolved " * eta * " across order representations")
  ) +
  guides(colour = "none") +
  theme_nature() +
  theme(
...
base_file <- file.path(out_dir, "Fig8_eta_order_representation_comparison")
...
write.csv(post_checks, file.path(out_dir, "Fig8_postplot_export_checks.csv"), row.names = FALSE)
...
write.csv(figure_numeric_summary, file.path(out_dir, "Fig8_figure_numeric_summary.csv"), row.names = FALSE)
...
writeLines(
  c(
    ...
    "Palette: Low-order #8B919A, High-order #5A84AF, DMW-HLG #D96A63, matching the restrained Fig. 7 method-highlight preference.",
  ...
  file.path(out_dir, "Fig8_figure_contract_and_audit_note.txt"),
  useBytes = TRUE
)

if (any(post_checks$status == "FAIL")) {
  stop("Export checks failed; inspect Fig8_postplot_export_checks.csv")
}
