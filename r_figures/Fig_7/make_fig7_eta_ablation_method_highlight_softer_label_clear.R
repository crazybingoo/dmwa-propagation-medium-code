...
base_file <- file.path(out_dir, "Fig7_eta_ablation_components_method_highlight_softer_label_clear")
...
write.csv(post_checks, file.path(out_dir, "Fig7_postplot_export_checks_method_highlight_softer_label_clear.csv"), row.names = FALSE)
...
writeLines(
  c(
    ...
    "Softer method-highlighting label-clear version: Fig7_eta_ablation_components_method_highlight_softer_label_clear.*",
    "Palette: Original #D96661, Coverage only #4778A8, DegreeBias only #7A828C.",
    "Rationale: the main method is foregrounded with a softer coral-red accent, while the ablated variants are visually pushed back with blue and gray. Panel-a pre-early labels are placed in the upper-left whitespace of each facet to match the current Fig. 8 label layout and avoid overlap with boxplots or whiskers.",
    "Data and statistics are unchanged from the audited Fig. 7 source tables."
  ),
  file.path(out_dir, "Fig7_palette_alignment_note_method_highlight_softer_label_clear.txt"),
  useBytes = TRUE
)

if (any(post_checks$status == "FAIL")) {
  stop("Export checks failed. Inspect Fig7_postplot_export_checks_method_highlight_softer_label_clear.csv")
}

message("Softer method-highlight label-clear Fig. 7 complete: ", base_file)
